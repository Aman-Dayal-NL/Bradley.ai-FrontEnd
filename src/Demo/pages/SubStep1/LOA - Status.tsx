...
const SubStep3: React.FC = () => {
  ...
  const getDetailsText = () => {
    ...
    switch (status) {
      case 'Approved':
        return `Congratulations! Your Letter of Authorization has been approved by ${company}. ${window.location.pathname.includes('/demo') ? 'EmissionCheckIQ+' : 'DERLabsIQ'} now has access to your interval data.`;
      case 'Awaiting Approval':
        return `Your Letter of Authorization has been signed and sent. It is now pending a response from ${company}. You will be notified via email confirmation once the interval data has been received by ${window.location.pathname.includes('/demo') ? 'EmissionCheckIQ+' : 'DERLabsIQ'}.`;
      case 'Declined':
